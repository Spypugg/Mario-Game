...
function drawBorders() {
  ...
  ctx.fillStyle = "black";
  ctx.fillText(
    `Score:  (PlaceHolder)         HighScore:  (PlaceHolder) `,
    75,
    cnv.height - 20
  );
...
// Increase Goomba Speed after time
requestAnimationFrame(goombaSpeedFunction);
function goombaSpeedFunction() {
  if (level1Timer < 120) {
    goombaSpeed = 1.7;
  } else if (level1Timer < 60) {
    goombaSpeed = 2;
  } else if (goombaSpeed >= 120) {
    goombaSpeed = 1.5;
  }
  requestAnimationFrame(goombaSpeedFunction);
}
...
function youWon() {
  ...
  ctx.font = "30px Arial";
  ctx.fillStyle = "black";
  ctx.fillText(`You Won, GREAT JOB, It took you ${tries} ${trys}`, 200, 300);

  setTimeout(resetFromWin, 3000);
}

function resetFromWin() {
  state === "start";
  tries = 1;
}
